"""
对比当前 scheme (15-10-75) 与主分析 (10-10-80) 的 RQ1 / RQ2 / RQ3 结果。
...
def diff_feature_importance(sort_by, kind):
    """对比 main (10-10-80) vs current (15-10-75) 的特征排名稳定性。
    ...
    # MI subset 不同, 用 inner join 只比较共选特征; 同时记录每边独有的
...
def write_md(rq1_results, rq2_df, rq3_data, feat_imp=None):
    ...
    md.append(f"> 当前 scheme 的 popular cut = 15%, gap cut = 10%, unpopular cut = 75% (top 15% 是 popular, top 15-25% 是 gap, 剩 75% 是 unpopular)。")
